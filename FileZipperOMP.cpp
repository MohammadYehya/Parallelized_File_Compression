...
void ZipFile()//Main Zip Function
{
	...
	else if(!FileName.compare(FileName.find('.'),4,".bmp"))
	{
		...
		int freq[N] = {0};
		int numThreads = 8;
		const int rowsPerThread = dheader.getHeight() / numThreads;
		unordered_map<unsigned char,unsigned int> hash;
		double t1 = omp_get_wtime();
		#pragma omp parallel num_threads(numThreads)
		{
			int threadID = omp_get_thread_num();
	        int startRow = threadID * rowsPerThread;
	        int endRow = (threadID == numThreads - 1) ? dheader.getHeight() : startRow + rowsPerThread;
				
			vector<uint8_t> rowData(PaddingBytes * (endRow - startRow));
			unordered_map<unsigned char, unsigned int> table;
			...
			{
				#pragma omp atomic
				hash[x.first] += x.second;
			}
		}
	    
		// Write pixel data
		for(auto &x : hash) List.Insert(CharInfo(x.second, x.first));
